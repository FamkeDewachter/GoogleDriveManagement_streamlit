...
def gds_get_all_subfolders(
    service, drive_id, folder_id=None, max_depth=3, search_term=None
):
    """
    Fetches all subfolders (up to a specified depth) under a given folder from a shared Google Drive.
    If no folder_id is provided, it fetches all root-level folders in the shared drive.
    If search_name is provided, only folders containing that name will be included.

    :param service: Authenticated Google Drive API service instance.
    :param drive_id: ID of the shared Google Drive.
    :param folder_id: (Optional) ID of the parent folder.
    :param max_depth: Maximum depth of subfolder traversal (default is 3).
    :param search_name: (Optional) Partial name of the folder to search for.

    :return: A list of matching subfolders within the parent folder and its descendants, up to the specified depth.
    """
    print("Fetching subfolders...")

    def _get_subfolders_recursive(
        service, drive_id, parent_id, folders, current_depth
    ):
        """
        Helper function to recursively fetch subfolders up to a specified depth.
        """
    ...
    # Initialize an empty list to store matching folders
    folders = []
    _get_subfolders_recursive(
        service, drive_id, folder_id, folders, current_depth=0
    )
